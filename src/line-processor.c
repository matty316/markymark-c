...
bool is_at_line_end(LineProcessor *lp) { return peek_char(lp) == '\0'; }
...
void add_code(LineProcessor *lp) {
  ...
  LineElement code_start = {.type = LE_CODE_OPEN};
  add_line_element(code_start, lp);

  LineElement te = {.start = text_start, .length = text_len, .type = LE_TEXT};

  add_line_element(te, lp);

  LineElement code_close = {.type = LE_CODE_CLOSE};
...
void add_link(LineProcessor *lp) {
  ...
  LineElement a_start = {.type = LE_LINK_START};
  LineElement a_mid = {.type = LE_LINK_MID};
  LineElement a_close = {.type = LE_LINK_CLOSE};

  LineElement te = {.start = text_start, .length = text_len, .type = LE_TEXT};

  LineElement he = {.start = href_start, .length = href_len, .type = LE_TEXT};
